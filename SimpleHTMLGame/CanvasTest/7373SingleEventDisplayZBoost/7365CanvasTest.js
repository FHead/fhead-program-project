...
var ViewWidth = SCREENWIDTH / 1000 * 2.5;
var ViewHeight = SCREENHEIGHT / 1000 * 2.5;
...
function Initialize()
{
   ...
   SCREENWIDTH = document.getElementById('Interface').style.width.replace('px', '');
   SCREENHEIGHT = document.getElementById('Interface').style.height.replace('px', '');

   ViewWidth = SCREENWIDTH / 1000 * 2.5;
   ViewHeight = SCREENHEIGHT / 1000 * 2.5;
   ...
   /*
   var ZGamma = 3500 / 0.938272046;
   var ZBetaGamma = Math.sqrt(ZGamma * ZGamma - 1);
   alert(ZGamma);
   alert(ZBetaGamma);
   for(ParticleIndex in ParticleList)
   {
      ...
      var P0 = Particle.P[0];
      var P3 = Particle.P[3];
      Particle.P[0] = P0 * ZGamma - P3 * ZBetaGamma;
      Particle.P[3] = -P0 * ZBetaGamma + P3 * ZGamma;
      
      var V0 = Particle.V0[0];
      var V3 = Particle.V0[3];
      Particle.V0[0] = V0 * ZGamma - V3 * ZBetaGamma;
      Particle.V0[3] = -V0 * ZBetaGamma + V3 * ZGamma;

      // alert(Particle.V0[0]);
   }
   */

   document.getElementById('SamplePanel').innerHTML = 'Sample = ' + SampleType;
...
function CalculateLocation(ReferenceTime, CurrentMilliSecond)
{
   ...
   for(ParticleIndex in ParticleList)
   {
      ...
      NewObject.X = XDistance / ViewWidth * SCREENWIDTH;
      NewObject.Y = YDistance / ViewHeight * SCREENHEIGHT;
...
function Render(CanvasHandle)
{
   ...
   CanvasHandle.fillRect(0, 0, SCREENWIDTH, SCREENHEIGHT);
...
function UpdateMouseXY(event)
{
   ...
   if(IE)
   {
      MouseX = event.clientX + document.body.scrollLeft - MouseShiftX;
      MouseY = event.clientY + document.body.scrollTop - MouseShiftY;
   }
   else
   {
      MouseX = event.pageX - MouseShiftX;
      MouseY = event.pageY - MouseShiftY;
   }
}
